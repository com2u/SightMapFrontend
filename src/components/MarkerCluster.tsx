...
const MarkerCluster = ({ locations }: Props) => {
  ...
  const filteredLocations = useMemo(() => {
    ...
    const data = locations.filter(item => selectedIcons.some(selectedIcon => selectedIcon.icon === item.icon));
    ...
    if (value && searchedData.length > 0) {
      map.flyTo([Number(searchedData[0].latitude), Number(searchedData[0].longitude)], 15);
    }
    return searchedData;
  ...
  return (
    <>
      ...
      <Filter setSelectedIcons={setSelectedIcons} selectedIcons={selectedIcons} />
